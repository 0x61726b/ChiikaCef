...
#define NEWLINE "\n"
	...
	ClientHandler::ClientHandler(Delegate* delegate,
		...
		CreateDelegates(DelegateSet_);
	}
	void ClientHandler::CreateDelegates(ProcessMessageDelegateSet& delegates)
	{
		Chiika_Browser::CreateProcessMessageDelegates(delegates);
	}
	...
	bool ClientHandler::OnProcessMessageReceived(
		...
		for (const auto &delegate : DelegateSet_)
		{
			if (delegate->IsAcceptedNamespace(message->GetName()))
			{
	...
	void ClientHandler::OnBeforeContextMenu(
		...
		if ((params->GetTypeFlags() & (CM_TYPEFLAG_PAGE | CM_TYPEFLAG_FRAME)) != 0) {
			...
			model->AddItem(CLIENT_ID_REFRESH_CACHE, "Refresh Ignore Cache");
	...
	void ClientHandler::NotifyTitle(const CefString& title) {
		...
		if (delegate_)
		{
			//Dont change the title throught the lifecycle
			delegate_->OnSetTitle("Chiika");
		}
	}
